...
def preprocess_image(img):
    ...
    image_img = cv2.cvtColor(sharpened, cv2.COLOR_BGR2YCrCb)
...
# ZeroMQ 통신 = REQ(request) & REP(reply) mode
context = zmq.Context()
socket = context.socket(zmq.REP) # socket open
...
try:
    while True:
        ...
        frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR) # numpy -> img decoding
        
        if frame is not None:     
            frame = cv2.resize(frame, (width, height), cv2.INTER_CUBIC) # 해상도 조절
            ...
            if result[0] is not None: # 텍스트 인식한 게 있다면
                for line in result[0]:
                    ...
                    cv2.putText(frame, str, (int(st_x), int(st_y+roi_y - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
